...
class SQLToken:
    ...
    @staticmethod
    def token2sql(token: Token, query: "query_module.BaseQuery") -> all_token_types:
        return next(SQLToken.tokens2sql(token, query))

    @staticmethod
    def placeholder_index(token) -> int:
        return int(
            re.match(r"%\(([0-9]+)\)s", token.value, flags=re.IGNORECASE).group(1)
        )
...
class AliasableToken(SQLToken):
    ...
    def is_explicit_alias(self):
        return len(self._token.tokens) == 5 and self._token[2].match(
            tokens.Keyword, "AS"
        )
...
class SQLColumnDef:
    ...
    def __init__(
        self, name: str = None, data_type: str = None, col_constraints: set = None
    ):
        self.name = name
    ...
    def def_str2col_def(cls, def_str: str):
        ...
        return SQLColumnDef(
            name=name, data_type=data_type, col_constraints=col_constraints
        )
...
ORDER_BY_MAP = {"ASC": ASCENDING, "DESC": DESCENDING}
