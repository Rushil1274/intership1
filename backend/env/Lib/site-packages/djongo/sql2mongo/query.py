...
@dataclass
class TokenAlias:
    alias2token: Dict[str, U[AliasableToken, SQLFunc, SQLIdentifier]] = dataclass_field(
        default_factory=dict
    )
    token2alias: Dict[U[AliasableToken, SQLFunc, SQLIdentifier], str] = dataclass_field(
        default_factory=dict
    )
...
class SelectQuery(DQLQuery):
    def __init__(self, *args):
        ...
        self.joins: List[U[InnerJoinConverter, OuterJoinConverter]] = []
    ...
    def _needs_aggregation(self):
        if self.nested_query or self.joins or self.distinct or self.groupby:
            return True
        if any(
            isinstance(sql_token, (SQLFunc, SQLConstIdentifier))
...
class InsertQuery(DMLQuery):
    ...
    def execute(self):
        ...
        auto = self.db["__schema__"].find_one_and_update(
            {"name": self.left_table, "auto": {"$exists": True}},
            {"$inc": {"auto.seq": num}},
        ...
        for i, val in enumerate(self._values):
            ...
            for _field, value in zip(self._cols, val):
                if (
                    auto
                    and _field in auto["auto"]["field_names"]
                    and value == "DEFAULT"
                ):
                    continue
                ins[_field] = value
...
class AlterQuery(DDLQuery):
    ...
    def _rename_column(self):
        self.db[self.left_table].update(
            {}, {"$rename": {self._old_name: self._new_name}}, multi=True
        )
    ...
    def _drop_column(self):
        self.db[self.left_table].update(
            {}, {"$unset": {self._iden_name: ""}}, multi=True
        )
        self.db["__schema__"].update(
            {"name": self.left_table}, {"$unset": {f"fields.{self._iden_name}": ""}}
        )

    def _add(self, statement: SQLStatement):
        for tok in statement:
            if tok.match(
                tokens.Keyword, ("CONSTRAINT", "KEY", "REFERENCES", "NOT NULL", "NULL")
            ):
                ...
                self.field_dir = [
                    (field.strip(' "'), 1) for field in tok.value.strip("()").split(",")
                ]
                ...
                raise SQLDecodeError(err_key=tok.value, err_sub_sql=statement)

    def _add_column(self):
        self.db[self.left_table].update(
            {"$or": [{self._iden_name: {"$exists": False}}, {self._iden_name: None}]},
            {"$set": {self._iden_name: self._default}},
            multi=True,
        )
        self.db["__schema__"].update(
            {"name": self.left_table},
            {"$set": {f"fields.{self._iden_name}": {"type_code": self._type_code}}},
        )

    def _index(self):
        self.db[self.left_table].create_index(self.field_dir, name=self._iden_name)

    def _unique(self):
        self.db[self.left_table].create_index(
            self.field_dir, unique=True, name=self._iden_name
        )
...
class CreateQuery(DDLQuery):
    ...
    def _create_table(self, statement):
        ...
        if not isinstance(tok, Parenthesis):
            raise SQLDecodeError(
                f"Unexpected sql syntax" f" for column definition: {statement}"
            )
        ...
        for col in SQLColumnDef.sql2col_defs(tok.value):
            if isinstance(col, SQLColumnConstraint):
                ...
                _set[f"fields.{field}"] = {"type_code": col.data_type}
                ...
                if (
                    SQLColumnDef.not_null in col.col_constraints
                    or SQLColumnDef.null in col.col_constraints
                ):
        ...
        if push:
            update["$push"] = push
        if update:
            self.db["__schema__"].update_one(filter=_filter, update=update, upsert=True)
...
class Query:
    ...
    def parse(self):
        logger.debug(f"sql_command: {self._sql}\n" f"params: {self._params}")
        ...
        try:
            ...
            try:
                ...
                exe = SQLDecodeError(
                    err_sql=self._sql, params=self._params, version=djongo.__version__
                )
                raise exe from e
