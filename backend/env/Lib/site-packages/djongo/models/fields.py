...
from django.db.models import Manager, Model, Field, AutoField, ForeignKey, BigAutoField
...
class ModelField(MongoField):
    ...
    def __init__(self, model_container: typing.Type[Model], *args, **kwargs):
    ...
    def _value_thru_fields(self, func_name: str, value: dict, *other_args):
        ...
        for field in self.model_container._meta.get_fields():
            try:
                ...
                continue
            processed_value[field.attname] = getattr(field, func_name)(
                field_value, *other_args
            )
    ...
    def _obj_thru_fields(self, func_name: str, obj: Model, *other_args):
        processed_value = {}
        for field in self.model_container._meta.get_fields():
    ...
    def get_db_prep_save(self, value, connection):
        ...
        if not isinstance(value, self.base_type):
            raise ValueError(f"Value: {value} must be an instance of {self.base_type}")

        processed_value = self._save_value_thru_fields(
            "get_db_prep_save", value, connection
        )
        return processed_value

    def get_prep_value(self, value):
        if value is None or not isinstance(value, self.base_type):
            return value

        processed_value = self._value_thru_fields("get_prep_value", value)
        return processed_value
...
class ArrayField(FormedField):
    ...
    def _value_thru_fields(
        self, func_name: str, value: typing.Union[list, dict], *other_args
    ):
        if isinstance(value, dict):
            return super()._value_thru_fields(func_name, value, *other_args)

        processed_value = []
        for pre_dict in value:
            post_dict = super()._value_thru_fields(func_name, pre_dict, *other_args)
            processed_value.append(post_dict)
        return processed_value

    def _save_value_thru_fields(
        self, func_name: str, value: typing.Union[list, dict], *other_args
    ):
        processed_value = []
        for pre_dict in value:
            post_dict = super()._save_value_thru_fields(
                func_name, pre_dict, *other_args
            )
            processed_value.append(post_dict)
        return processed_value
...
class ArrayFormField(forms.Field):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(error_messages=error_messages, widget=widget, *args, **kwargs)
...
class EmbeddedFormWidget(forms.MultiWidget):
    ...
    def value_from_datadict(self, data, files, name):
        ...
        for i, wid in enumerate(self.widgets):
            ...
            ret.append(wid.value_from_datadict(data, files, f_n))
        return ret
...
class ObjectIdField(_ObjectIdField):
    ...
    def __init__(self, *args, **kwargs):
        id_field_args = {"primary_key": True, "auto_created": True}
...
class ArrayReferenceManagerMixin:
    ...
    def update_or_create(self, **kwargs):
        ...
        obj, created = super(
            ArrayReferenceManagerMixin, self.db_manager(db)
    ...
    def create(self, **kwargs):
        db = router.db_for_write(self.instance.__class__, instance=self.instance)
        new_obj = super(ArrayReferenceManagerMixin, self.db_manager(db)).create(
            **kwargs
        )
        self.add(new_obj)
        return new_obj
...
def create_reverse_array_reference_manager(superclass, rel):
    ...
    class ReverseArrayReferenceManager(ArrayReferenceManagerMixin, baseclass):
        ...
        def _make_filter(self, *objs):
            ...
            return {self.model._meta.pk.name: {"$in": list(ids)}}
        ...
        def add(self, *objs):
            ...
            self.mongo_update(
                ...
                {
                    "$addToSet": {
                        lh_field.get_attname(): getattr(
                            self.instance, rh_field.get_attname()
                        )
                    }
                },
            )
    ...
    class ArrayReferenceManager(ArrayReferenceManagerMixin, baseclass):
        ...
        def add(self, *objs):
            ...
            self.instance_manager.db_manager(db).mongo_update(
                self._make_filter(),
                {"$addToSet": {self.field.attname: {"$each": list(new_fks)}}},
            )
        ...
        def _remove(self, to_del):
            ...
            self.instance_manager.db_manager(db).mongo_update(
                self._make_filter(),
                {"$pull": {self.field.attname: {"$in": list(to_del)}}},
            )

        def clear(self):
            db = router.db_for_write(self.instance.__class__, instance=self.instance)
            self.instance_manager.db_manager(db).mongo_update(
                self._make_filter(), {"$set": {self.field.attname: []}}
            )
